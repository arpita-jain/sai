...
function getbt_search()
{
    ...
    require_once ABSPATH.'wp-content/plugins/join-network/template/tb_search.php';
}
...
function joinnow_login_form_verifyemail() {
			...
			global $errors;
			global $wpdb;
			if ( isset( $_GET['verification_code'] ) && '1' === $joinnowoption->joinow_get_option( 'verify_user_email' ) ) {
...
function joinnow_my_verified()
{
      ?>
				...
				$(document).ready(function() {
				   //alert("dfsfs");
					$(".content-left-part").html("You account has been verified.");
				});
				</script>
				<?php
...
function apply_trader_fromuser()
{
     ...
     // $job_title = $wpdb->get_var("SELECT job_title FROM wp_jobpost where id=$job_id");
      //$sender = $wpdb->get_var( "SELECT user_login FROM $wpdb->users WHERE id = '$user_recepitent_id' LIMIT 1" );
	     ...
	     $content='<span>Your request  has been sent to '.$all_meta_for_user['first_name'][0].'</span>'; 
	    				$new_message = array(
					'id'        => NULL,
					'subject'   => $subject,
					'content'   => $content,
