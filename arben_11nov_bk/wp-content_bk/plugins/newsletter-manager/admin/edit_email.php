...
$_POST = xyz_trim_deep($_POST);
		...
		if(is_email($xyz_em_email)){
			$email_count = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'xyz_em_email_address WHERE id!="'.$xyz_em_emailId.'" AND email="'.$xyz_em_email.'" LIMIT 0,1' ) ;
			// 				echo '<pre>';
			// 				print_r($email_count);
			// 		die;
			if(count($email_count) == 0){

				$nameCount = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'xyz_em_additional_field_value WHERE ea_id="'.$xyz_em_emailId.'" ' ) ;
				...
				if(count($nameCount) > 0){
					...
					if($xyz_em_name=='')
						$wpdb->query( 'delete  FROM '.$wpdb->prefix.'xyz_em_additional_field_value WHERE ea_id="'.$xyz_em_emailId.'" ' ) ;
...
$emailres = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'xyz_em_email_address WHERE id="'.$xyz_em_emailId.'" LIMIT 0,1' ) ;
...
if(count($emailres)==0){
	...
	$nameres = $wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'xyz_em_additional_field_value WHERE ea_id="'.$xyz_em_emailId.'" ' ) ;
