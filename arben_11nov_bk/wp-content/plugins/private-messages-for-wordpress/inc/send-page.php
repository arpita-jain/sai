...
function rwpm_front_message_send()
{
	global $wpdb, $current_user;
...
<div class="msgcontainer">
    <h2 class="headmail"><?php _e( 'Compose New Messageeeeee', 'pm4wp' ); ?></h2>
	<?php
	$option = get_option( 'rwpm_option' );
	if ( $_REQUEST['page'] == 'rwpm_front_message_send' && isset( $_POST['submit'] ) )
	{
		$error = false;
		$status = array();
//echo "madhuri"; die("coming here");
		// Check if total pm of current user exceed limit
		$role = $current_user->roles[0];
		$sender = $current_user->user_login;
		$total = $wpdb->get_var( 'SELECT COUNT(*) FROM ' . $wpdb->prefix . 'pm WHERE `sender` = "' . $sender . '" OR `recipient` = "' . $sender . '"' );
		...
		if ( !$error )
		{
			...
			$status[] = sprintf( _n( '%d message sent.', '%d messages sent.', $numOK, 'pm4wp' ), $numOK );}

		echo '<div id="message" class="sucess_msg" class="updated fade"><p>', implode( '</p><p>', $status ), '</p></div>';
	}
	?>
	<?php do_action( 'rwpm_before_form_send' ); ?>
    <form method="post" action="" id="send-form" enctype="multipart/form-data">
	    <input type="hidden" name="page" value="rwpm_front_message_send" />
        <table class="form-table">
            <tr>
                <th><?php _e( 'Recipient', 'pm4wp' ); ?></th>
                <td>
...
/********************for Mob_view******************/
function rwpm_front_message_send_mobile()
{
	global $wpdb, $current_user;
	include("ckeditor/ckeditor.php");
	$CKEditor = new CKEditor();
	...
	?>
<div class="msgcontainer">
    <h2 class="headmail"><?php //_e( 'Composeeeeeee New Message', 'pm4wp' ); ?></h2>
	<?php
//	$option = get_option( 'rwpm_option' );
//	if ( $_REQUEST['page'] == 'rwpm_front_message_send' && isset( $_POST['submit'] ) )
//	{
