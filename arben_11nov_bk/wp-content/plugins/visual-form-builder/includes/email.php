...
//mail to builder
 global $current_user;
	 ...
	 $num=0;
	 foreach($data as $quote_data)
	 {
		$imp_quote_data[]=$data[$num]['value'];
		$num++;
	 }
	 $br_string= $imp_quote_data['5'];
	$add= str_replace('&lt;br&gt;', " ", $br_string);
	//echo $add;
	 //print_r($imp_quote_data);
	 //die();
	  $id=$_REQUEST['id'];
	 $userdata = get_userdata($id);
	 $to_mail=$userdata->user_email;
	// print_r($to_mail);
	//die();
	 $f_subject="test";
	 $message="<table>
	 <tr><td>Name:</td><td>".$imp_quote_data['1']."</td></tr> <tr>
	 <td>Contact Number:</td><td>".$imp_quote_data['2']."</td></tr>
	 <tr><td>Email ID:</td><td>".$imp_quote_data['3']."</td></tr>
	 <tr><td>URL:</td><td>".$imp_quote_data['4']."</td></tr>
	 <tr><td>Address:</td><td>".$add."</td></tr>
	 <tr><td>Description:</td><td>".$imp_quote_data['6']." ".$imp_quote_data['7']." ".$imp_quote_data['8']."</td></tr> </table>";
	 $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
            $headers .= 'From: test.mail@gmail.com' . "\r\n";
	wp_mail( $to_mail, $f_subject, $message, $headers, '' );
	
	//mail to user
	$user_q_mailid=$user_mail;
	$user_subject="quote confirmation";
	 $user_message="Your quote has been sent successfully";
	 $user_headers = "MIME-Version: 1.0" . "\r\n";
            $user_headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
            $user_headers .= 'From: test.mail@gmail.com' . "\r\n";
	wp_mail( $user_q_mailid, $user_subject, $user_message, $user_headers, '' );
