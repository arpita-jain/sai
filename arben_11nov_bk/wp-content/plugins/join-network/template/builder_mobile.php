<?php  include_once(ABSPATH.'wp-content/plugins/job-post/pagination.class.php');
	    global $current_user;
	    ...
	    global $wpdb;
	     ...
	     $q1 = "SELECT  distinct a.user_id FROM wp_usermeta as a JOIN wp_usermeta as b JOIN wp_usermeta as c  ON a.meta_key='usertype' and a.user_id=b.user_id and b.user_id = c.user_id and a.meta_value='3'  $condition  $condition2 order by user_id desc";
	     }
	     
	     else
	     {
		  $q1 = "SELECT * FROM wp_usermeta where meta_key='usertype' and meta_value='3' order by user_id desc";
	     }
	   ...
	   $items = count($retrieve_data); // number of total rows in the database
        ...
        $p->target(site_url()."?page_id=11&theme=handheld");
     ...
     //   $show = $p->show();
	  ...
	  else
	  {
	      ...
	      global $wpdb;
	   //  $retrieve_data = $wpdb->get_results("SELECT * FROM wp_usermeta where meta_key='usertype' and meta_value='3' $limit");
?>
...
<?php
	     ...
	     $q1 = "SELECT  distinct a.user_id FROM wp_usermeta as a JOIN wp_usermeta as b JOIN wp_usermeta as c  ON a.meta_key='usertype' and a.user_id=b.user_id and b.user_id = c.user_id and a.meta_value='3'  $condition  $condition2 order by user_id desc $limit";
	     }
	     
	     else
	     {
		  $q1 = "SELECT * FROM wp_usermeta where meta_key='usertype' and meta_value='3' order by user_id desc $limit";
	     }
	      $retrieve_data = $wpdb->get_results($q1);
	     // echo count($retrieve_data);
?>
<div class="tabs_search">
 <div class="build_search_field_main">
<form action="<?php echo site_url().'?page_id=11&theme=handheld'?>" method="post" name="searchform" id="searchform">
    <div class="build_search_field_main">
...
<ul class="tab_2list">
		...
		$cont=substr($about_company, 0, 90);?>
          ...
          <span class="top_s"><?php echo strtolower($cont);?></span>
            </div>
	    <?php
