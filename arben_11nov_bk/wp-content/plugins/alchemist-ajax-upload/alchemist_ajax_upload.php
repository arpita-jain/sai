...
class Alchimest__Ajax_Image_Upload
{
    ...
    ?>
    <script type="text/javascript" src="<?php echo site_url();?>/wp-content/plugins/join-network/js/jquery-1.9.1.min.js?ver=3.5.1"></script>
    <?php
        ...
        wp_enqueue_script('aaiu_upload', AAIU_URL . 'js/aaiu_upload.js', array('jquery'));?>
          <script>
            jQuery(document).ready(function ($) {
                     var selectedcat;
                     $('.checkedradio').unbind('click');
                     $('.checkedradio').click(function(){
                    
                    selectedcat= $("input:radio[name=img_category]:checked").val();
                    // alert(this.id);
                     ret = uploadLimit(selectedcat,'upload');
                    //if(!(ret)){ 
                    //  ret1 = $("#upl_upload").html();
                    //  if(ret1)
                    //   {
                    //    $("#aaiu-uploader").hide();
                    //   }
                    ...
                    //    ret1 = $("#upl_upload").html();
                    //    } 
                     $.post("<?php echo admin_url('admin-ajax.php')?>",{action:"set_checkedvalue_action",category:selectedcat},function(data){
                     //alert(data);
                        });
       ...
       <?php  wp_localize_script('aaiu_upload', 'aaiu_upload', array(
 ...
 public function set_checkedvalue()
    {
       session_start();
      echo $_SESSION['selectcategory']=$_REQUEST['category'];
    }

    function handle_file($upload_data)
    {
         session_start();
     $category= $_SESSION['selectcategory'];
     $user_ID = get_current_user_id();
        ...
        if (isset($uploaded_file['file'])) {
            ...
            add_post_meta($attach_id,'category',$category);
             add_post_meta($attach_id,'user',$user_ID); 
    ...
    function getHTML($attachment)
    {
        ...
        $html = '';
       $html .= '<span class="aaiu-uploaded-files clearfix">';
...
add_action('wp_ajax_set_checkedvalue_action', array($aaui, 'set_checkedvalue'));

/* For non logged-in user */
add_action('wp_ajax_nopriv_aaiu_upload', array($aaui, 'upload'));
add_action('wp_ajax_nopriv_aaiu_delete', array($aaui, 'delete_file'));
add_action('wp_ajax_nopriv_set_checkedvalue_action', array($aaui, 'set_checkedvalue'));
