<?php
if(!class_exists('WpBpShop_Settings_API')) :
class WpBpShop_Settings_API {
    ...
    function admin_init() {
        ...
        foreach ((array)$this->settings_sections as $section) {
        ...
        foreach ((array)$this->settings_fields as $section => $field) {
            foreach ((array)$field as $option) {
                $args = array(
                    'id' => $option['name'],
                    ...
                    'before' => $option['before'] ? $option['before'] : '',
                    'after' => $option['after'] ? $option['after'] : '',
                );
    ...
    function callback_multicheck( $args ) {
        ...
        $html .= sprintf( '<br><span class="description"> %s</label>', $args['desc'] );
        $html .= $args['after'];
        echo $html;
    }
    ...
    /**
     * Displays a hidden for a settings field
     *
     * @param array $args settings field args
     */
    function callback_hidden( $args ) {
        ...
        $html = sprintf( '<input type="hidden" class="%1$s-text%5$s" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $args['value'], $args['class'] );
    ...
    function show_navigation() {
        ...
        foreach ((array)$this->settings_sections as $tab) {
    ...
    function show_forms() {
        ...
        <div class="metabox-holder">
            <div class="postbox">
                <?php foreach ((array)$this->settings_sections as $form) { ?>
