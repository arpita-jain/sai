...
class Wpjb_Form_Admin_Resume extends Daq_Form_ObjectAbstract
{
    ...
    public function init()
    {
        if( mbt_get_user_type() == 'artist' ) {
            ...
            $e->setHint(__('This field will be shown only to registered studios and artists.', WPJB_DOMAIN));
            ...
            $e->setHint(__("If resume is inactive studios won't find you search results.", WPJB_DOMAIN));
...
<?php }     
             // $e = new Daq_Form_Element("up_img", Daq_Form_Element::TYPE_SUBMIT);
             // $e->setLabel(__("Button try", WPJB_DOMAIN));
             // $e->setHint(__("If resume is inactive studios won't find you search results.", WPJB_DOMAIN));
             ////$e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
             // $e->setValue($this->_object->up_img);
             //$e->addFilter(new Daq_Filter_Int());
             //$this->addElement($e, "resume");
            
            $e = new Daq_Form_Element_File("image", Daq_Form_Element::TYPE_FILE);
            $e->setLabel(__("Your Photooooo", WPJB_DOMAIN));
            $e->setHint(__("Max. file size 300 kB. File formats *.jpg; *.gif; *.png .", WPJB_DOMAIN));
...
?>
           ...
           <?php $e = new Daq_Form_Element("title");
            $e->setLabel(__("Professional Headline", WPJB_DOMAIN));
            $e->setRequired(true);
            $e->setHint(__("Describe yourself in few words", WPJB_DOMAIN));
            $e->addValidator(new Daq_Validate_StringLength(1, 120));
            $e->setValue($this->_object->title);
            $e->setPlaceholder(__("I am a very creative Artist with good taste ...", WPJB_DOMAIN));
            //$e->addClass("wpjb-auto-clear");
            $this->addElement($e, "resume");
            if (mbt_get_sub_user_type() != 'p_artist') {
                $e = new Daq_Form_Element("category_id", Daq_Form_Element::TYPE_CHECKBOX);
                $e->setLabel(__("Primary Style", WPJB_DOMAIN));
                $values = maybe_unserialize($this->_object->category_id);
                //print_r($this->_object->category_id);
    ...
    public function save()
    {
        ...
        if($file && $file->fileSent()) {
            ...
            $file->upload("photo_".$this->getObject()->getId().".".$file->getExt());
        }
