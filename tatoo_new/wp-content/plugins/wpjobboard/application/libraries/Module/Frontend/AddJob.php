...
class Wpjb_Module_Frontend_AddJob extends Wpjb_Controller_Frontend
{
    ...
    private function _canPost()
    {
        ...
        if( !is_user_logged_in() ) {
            $this->view->_flash->addError(__("Please login before posting jobs", WPJB_DOMAIN));
            $this->view->canPost = false;
            $this->view->can_post = false;
            return false;
        }

        if( mbt_get_user_type() != "studio" ) {
            $this->view->_flash->addError(__("Only Studios can post jobs", WPJB_DOMAIN));
            $this->view->canPost = false;
            $this->view->can_post = false;
            return false;
        }
    ...
    public function addAction()
    {
        ...
        } elseif($empId > 0 && $job->employer_id == $empId) {
            $arr = $job->allToArray();
            if($job->hasImage()) {
