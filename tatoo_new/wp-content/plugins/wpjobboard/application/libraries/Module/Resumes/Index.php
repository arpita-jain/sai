...
class Wpjb_Module_Resumes_Index extends Wpjb_Controller_Frontend
{
    ...
    public function searchAction()
    {
        ...
        if($request->get("degree")) {
            $val = $request->get("degree");
            $query->where("degree = ?", $val);
        }
        
        if($request->get("experience")) {
            $val = $request->get("experience");
            $query->where("years_experience = ?", $val);
        }

        if($request->get("posted")) {
            $days = $request->get("posted");

            if($days == 1) {
                $time = date("Y-m-d");
                $query->where("DATE(updated_at) = ?", date("Y-m-d"));
            } elseif($days == 2) {
                $time = date("Y-m-d", strtotime("yesterday"));
                $query->where("DATE(updated_at) = ?", date("Y-m-d", strtotime("now -1 day")));
            } else {
                $query->where("updated_at >= DATE_SUB(NOW(), INTERVAL ? DAY)", (int)$days);
            }
        }
