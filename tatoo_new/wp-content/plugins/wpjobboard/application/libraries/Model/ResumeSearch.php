    ...
    public static function search($query, $category, $degree, $experience, $posted, $count, $page)
    {
        ...
        if (is_array($category)) {
            ...
            foreach($category as $key => $val) {
                if (!empty($val)) {
                    if ($i==0 && $i!=(count($category)-1)) $select->where('( category_id REGEXP \'i:[[:digit:]]+;s:[[:digit:]]+:"'.$val.'"\'');
                    else if ($i==0 && $i==(count($category)-1)) $select->where('category_id REGEXP \'i:[[:digit:]]+;s:[[:digit:]]+:"'.$val.'"\'');
                    else if ($i!=0 && $i==(count($category)-1)) $select->orWhere('category_id REGEXP \'i:[[:digit:]]+;s:[[:digit:]]+:"'.$val.'"\' )');
                    else $select->orWhere('category_id REGEXP \'i:[[:digit:]]+;s:[[:digit:]]+:"'.$val.'"\'');
                }
                $i++;
            }
        }
