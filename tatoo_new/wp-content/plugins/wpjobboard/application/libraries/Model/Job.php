...
class Wpjb_Model_Job extends Daq_Db_OrmAbstract
{
    ...
    public function custom_save()
    {
        ...
        if($this->getId() > 0) {
            ...
            if (count($_POST) > 0) {
                if (!array_key_exists('job_category', $_POST)) {
                    $toSave['job_category'] = serialize(array());
                }
            } else if ( !array_key_exists('job_category', $this->_field) ) {
                $toSave['job_category'] = serialize(array());
            }
            ...
            if (array_key_exists('job_category', $this->_field)) {
                if (!array_key_exists('value', $this->_field['job_category']) || !$this->_field['job_category']['value'])
                    $toSave['job_category'] = serialize(array());
            }
    ...
    public function getCategory($param) {
        ...
        //if(!isset($this->_loaded[$value['foreign']])) {
            if($param == true) {
                ...
                $final_result = array();
                if(!empty($result) && count($category_ids)) {
