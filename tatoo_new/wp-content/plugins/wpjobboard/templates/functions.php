...
{
    $categories = wpjb_view("job")->getCategory(true);
    //print_rr($categories);
    $links = array();
    foreach ($categories as $category) {
        $title = esc_html($category->title);
        $links[] = '<a href="'.wpjb_link_to("category", $category).'">'.$title.'</a>';
    }
    echo join(', ', $links);
}
...
{
    $type = wpjb_view("job")->getType(true);
...
{
    $steps = array(
        array(esc_html(Wpjb_Project::getInstance()->conf("seo_step_1")), __("Create", WPJB_DOMAIN)),
...
function wpjb_company_website(Wpjb_Model_Employer $company, $default = "")
{
    ...
    if(empty($website)) {
        ...
        echo "<a href=\"{$website}\" class=\"wpjb-company-link\" target=\"_blank\">{$website}</a>";
    }
}
