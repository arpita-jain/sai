...
class Daq_Db_Query
{
    ...
    public function getQueryClause($clause) {
        return $this->{'_'.$clause};
    }
    ...
    public function fetchCount($sql = '')
    {
        ...
        if (!empty($sql)) {
              ...
              $sql = preg_replace('/SELECT ([a-z0-9\s\.\_\-\,]+) FROM/i', 'SELECT COUNT(*) AS cnt FROM', $sql);

              if (stripos($sql, '#WHERE#') !== false) {
                if ( $this->_where ) {
                  $sql = str_ireplace('#WHERE#', 'WHERE '.$this->_where, $sql);
                } else {
              ...
              if (stripos($sql, '#ORDER#') !== false) {
                $sql = str_ireplace('#ORDER#', $this->_order, $sql);
              }
              ...
              if (stripos($sql, 'GROUP BY') !== false) {
                //$sql = preg_replace('/GROUP BY([a-z0-9\s\.\_\-\,]+)/i', '', $sql);
              }
    ...
    public function fetchAll($sql = '')
    {
        ...
        if (!empty($sql)) {
              ...
              if (stripos($sql, '#LIMIT#') !== false) {
                $sql = str_ireplace('#LIMIT#', $this->_limit, $sql);
              }
