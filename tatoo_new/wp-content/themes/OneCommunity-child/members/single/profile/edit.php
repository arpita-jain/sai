...
$groups = mbt_profile_groups_exclude_from_tab();
if ( count($groups) < 1 ) { $groups = BP_XProfile_Group::get( array( 'fetch_fields' => true ) ); }

$groups_id_array = array();
foreach( $groups as $item ) :
	$groups_id_array[] = "'#xprofile_form_" . $item->id . "'";
endforeach;
if ( count($groups_id_array) > 0 ) {
	$groups_id_array = join(", ", $groups_id_array);
}
?>
<h4>Edit Profile</h4>
<div id="message"></div>
<input type="button" value="Save Changes" class="submit profile_button" onclick="mbt_send_requests( Array(<?php echo $groups_id_array; ?>) )" />

<script>
	var num_responses		= Array();
	var total_requests		= 0;
	var current_requests	= 0;
	var error				= 0;
	var message				= '';
	function mbt_send_profile_data(form_ref) {
		var noti 	= jQuery('#message');
		var url		= ajaxurl + '?action=save_xprofile_data';
		var form	= jQuery(form_ref);
		var data 	= form.serialize();
		jQuery.post(url, data, function(response) {
			response = eval('(' + response + ')');
			num_responses[current_requests] 			= Array();
			num_responses[current_requests]['error'] 	= response.error;
			num_responses[current_requests]['message'] 	= response.message;
			num_responses[current_requests]['form'] 	= form_ref;
			if ( response.error == 1 ) { error = 1; message = response.message; }
			current_requests++;

			if ( current_requests == total_requests ) {
				if ( error == 1 ) {
					jQuery(noti).html('<p>' + message + '</p>');
					jQuery(noti).addClass('error');
					jQuery(noti).slideDown('fast');
				} else {
					jQuery(noti).html('<p>' + response.message + '</p>');
					jQuery(noti).removeClass('error');
					jQuery(noti).slideDown('fast');
				}
				jQuery('.profile_button').val('Save Changes');
				jQuery('.profile_button').removeAttr('disabled');
			}
		});
	}
	...
	function mbt_send_requests( groups ) {
		error = 0
		message = '';
		jQuery('.profile_button').val('Please wait...');
		jQuery('.profile_button').attr('disabled', true);
		var noti 	= jQuery('#message');
		jQuery(noti).slideUp('fast');
		current_requests = 0;
		total_requests = groups.length;
		for( var i = 0; i < groups.length; i++ ) {
			jQuery(groups[i]).submit();
		}
	}
</script>

<?php 
while ( $counter < count($groups) ) :
	$group_id = $groups[$counter]->id;
if ( bp_has_profile( 'profile_group_id=' . $group_id ) ) :
	$counter++;
	while ( bp_profile_groups() ) : bp_the_profile_group(); ?>

<form onsubmit="mbt_send_profile_data(this); return false;" id="xprofile_form_<?php echo $group_id; ?>" action="<?php bp_the_profile_group_edit_form_action(); ?>" method="post" id="profile-edit-form" class="standard-form <?php bp_the_profile_group_slug(); ?> bp-profile-form">
